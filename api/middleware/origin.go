...
func OriginMiddleware(origin string) gin.HandlerFunc {
	return func(c *gin.Context) {
		if c.Request.Method != http.MethodGet && c.Request.Method != http.MethodHead && c.GetHeader("Origin") != origin {
...
func RecoveryMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		defer func() {
			if r := recover(); r != nil {
				logging.Errorc(c.Request.Context(), "panic while handling %s: %v", c.Request.URL.Path, r)
