...
export function useRoundHistory(scope: Partial<RoundFilters> = {}) {
  ...
  const filters = ref<RoundFilters>({
    from: '',
    to: '',
    game: '',
    player: '',
  ...
  async function load(more = false) {
    ...
    error.value = ''
    try {
      const result = await listRounds(groupId, {
        ...filters.value,
        ...scope,
        limit: 30,
        offset: more ? page.value.items.length : 0,
      })
  ...
  function apply(value: RoundFilters) {
    filters.value = { ...value, ...scope }
    void load()
  }
  onMounted(() => {
    void load()
