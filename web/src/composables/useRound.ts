...
export function useRound(id: string) {
  ...
  async function load() {
    ...
    try { const round = await getRound(groupId, id); if (run === generation) current.value = round }
    catch (cause) { if (run === generation) { error.value = message(cause); await handleAccessError(cause) } }
