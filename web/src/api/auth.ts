...
export const sendLoginCode = (email: string, invite: string, groupToken = '') =>
  send<void>('/auth/code', { email, invite: groupToken ? '' : invite, group_token: groupToken })
export const login = (email: string, code: string, groupToken = '') =>
  send<LoginResult>('/auth/login', { email, code, group_token: groupToken })
