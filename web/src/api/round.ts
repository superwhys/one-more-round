...
export const getRoundShareStatus = (groupId: string, id: string) => request<RoundShareStatus>(`/groups/${groupId}/rounds/${id}/share`)
export const createRoundShare = (groupId: string, id: string) => send<RoundShareToken>(`/groups/${groupId}/rounds/${id}/share`, {})
export const revokeRoundShare = (groupId: string, id: string) => send<void>(`/groups/${groupId}/rounds/${id}/share`, {}, 'DELETE')
export const getPublicRound = (token: string) => request<PublicRound>('/shared-rounds', { headers: { 'X-Round-Share': token } })
export async function getPublicRoundPhoto(token: string, id: string): Promise<Blob> {
  let response: Response
  try {
    response = await fetch(`/api/v1/shared-rounds/photos/${encodeURIComponent(id)}`, { headers: { Accept: 'image/jpeg', 'X-Round-Share': token }, signal: AbortSignal.timeout(10_000) })
  } catch {
