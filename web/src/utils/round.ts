...
export function emptyRound(): Round { return { id: '', game_id: '', date: today(), mode: 'individual', outcome: '', players: [], winners: [], scores: {}, teams: [], team_score: null, memory: '', location: '', minutes: null, photos: [], author: '', updated_by: '', updated_at: '0001-01-01T00:00:00Z', deleted_at: null, version: 0 } }
export function resultLabel(r: Round, players: Player[]): string {
  ...
  if (r.mode === 'team') return `♔ ${r.teams.filter(t => t.winner).map(t => t.name).join('、')}获胜`
