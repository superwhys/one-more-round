...
function create() {
  return run(async () => {
    const group = await createGroup(name.value, playerName.value)
...
function join() {
  ...
  return run(async () => {
    ...
    try {
      ...
      session.error.value = '已加入小组，但列表加载失败，请重新加载'
    }
...
function inspect() {
  joinToken.value = tokenInput.value.trim()
  error.value = ''
}
function cancel() {
  session.clearInvitations()
  tokenInput.value = ''
  error.value = ''
  if (selected.value) void router.push('/')
}
</script>
  ...
  <section class="j-auth d-surface">
    ...
    <template v-if="joinToken">
      ...
      <template v-if="preview"
        ><h2>{{ preview.name }}</h2>
        <p>{{ alreadyMember ? '你已经是小组成员，可以直接进入。' : '加入后可以查看组内回忆，也可以记录新的对局。' }}</p>
        <button class="d-button full" :disabled="busy" @click="join">
          {{ busy ? '请稍候…' : alreadyMember ? '进入小组' : '加入小组' }}
        </button></template
      >
    ...
    <template v-else>
      ...
      <form @submit.prevent="create">
        <label class="d-field"
          >小组名称<input v-model="name" maxlength="255" required placeholder="比如：周五不散场" /></label
        ><label class="d-field"
          >你的玩家昵称<input v-model="playerName" maxlength="255" required placeholder="大家平时怎么称呼你？" /><small
            >会创建并关联为你在这个小组里的玩家档案。</small
          ></label
        ><button class="d-button full" :disabled="busy">创建小组</button>
      </form>
      <hr />
      <form @submit.prevent="inspect">
        <label class="d-field">小组邀请码<input v-model="tokenInput" required autocomplete="off" /></label>
        <p class="d-note">也可以直接打开朋友分享的小组邀请链接。</p>
        <button class="d-button secondary full" :disabled="busy || !tokenInput.trim()">查看邀请</button>
      </form>
