...
function save(round: Round, key: string) {
  return run(async () => {
    saved.value = await updateRound(groupId, id, round, key)
    await router.push(`/rounds/${saved.value.id}`)
  })
}
</script>

<template>
  <RequestStatus :loading="loading" :error="loadError" @retry="load" />
  <p v-if="notice" class="j-notice" role="status">{{ notice }}</p>
  <RoundForm
    v-if="snapshot && user && current"
    :snapshot="snapshot"
    :user="user"
    :editing="current"
    :saving="busy"
