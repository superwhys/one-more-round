...
const result = computed(() => {
  ...
  if (round.value.mode === 'team') return `♔ ${round.value.teams.filter(team => team.winner).map(team => team.name).join('、')}获胜`
...
<template>
  <div class="journal-app journal-root j-public-share">
    <header class="j-public-header"><img :src="favicon" alt="" /><span>又一局<small>记下每一局的输赢与相聚</small></span></header>
    <main class="j-public-main">
      <RequestStatus :loading="loading" :error="error" @retry="load" />
      <template v-if="round">
        <header class="d-page-heading compact">
          <div><p class="d-eyebrow">{{ round.group_name }} 分享的桌边回忆</p><h1>{{ round.game_name }}</h1><p>{{ round.date }} · {{ modeNames[round.mode] }}</p></div>
        </header>
        <article class="d-surface j-detail">
          <h2 class="d-result" :class="round.outcome">{{ result }}</h2>
          <div class="j-players">
            <span v-for="player in round.players" :key="player.name" class="j-player"><PlayerChip :name="player.name"><span v-if="player.score != null">{{ player.score }} 分</span></PlayerChip></span>
          </div>
          <div v-for="team in round.teams" :key="team.name" class="j-team"><strong>{{ team.name }} {{ team.winner ? '♔ 获胜' : '' }}</strong><p>{{ team.players.join('、') }}<span v-if="team.score != null"> · {{ team.score }} 分</span></p></div>
          <p v-if="round.team_score != null">团队分数：{{ round.team_score }}</p>
          <blockquote v-if="round.memory">{{ round.memory }}</blockquote>
          <div v-if="photoURLs.length" class="j-public-photos"><img v-for="(url, index) in photoURLs" :key="round.photos[index]" :src="url" alt="分享的对局照片" /></div>
        </article>
      </template>
    </main>
