...
<template>
  ...
  <template v-if="snapshot && current">
    ...
    <section class="d-surface j-detail">
      ...
      <div class="j-players">
        <RouterLink v-for="id in current.players" :key="id" :to="`/players/${id}`" class="j-player">
          <PlayerChip :name="playerName(id)"><span v-if="current.scores?.[id] != null">{{ current.scores[id] }} 分</span></PlayerChip>
        </RouterLink>
      </div>
      <div v-for="team in current.teams" :key="team.id" class="j-team">
        <strong>{{ team.name }} {{ team.winner ? '♔ 获胜' : '' }}</strong>
        <p>{{ team.players.map(playerName).join('、') }}<span v-if="team.score != null"> · {{ team.score }} 分</span></p>
      </div>
      <p v-if="current.team_score != null">团队分数：{{ current.team_score }}</p>
      <blockquote v-if="current.memory">{{ current.memory }}</blockquote>
      <div class="j-photos">
        <button v-for="id in current.photos" :key="id" @click="fullPhoto = id; modal = 'photo'"><img :src="photoURL(groupId, id)" alt="放大查看对局照片" /></button>
      </div>
      <p v-if="current.location || current.minutes">{{ current.location }} {{ current.minutes ? `· ${current.minutes} 分钟` : '' }}</p>
      <p class="d-note">记录人：{{ memberName(current.author) }}<br />最近修改：{{ memberName(current.updated_by) }} · {{ new Date(current.updated_at).toLocaleString('zh-CN', { timeZone: 'Asia/Shanghai' }) }}（北京时间）</p>
    ...
    <Modal v-if="modal === 'delete'" title="删除这段对局记录？" @close="modal = ''">
      ...
      <div class="d-modal-actions"><button class="d-button secondary" @click="modal = ''">再想想</button><button class="d-button" :disabled="busy" @click="deleteRound">删除这局</button></div>
    </Modal>
    <Modal v-if="modal === 'share'" title="把这一局分享出去" @close="modal = ''">
      <p>获得链接的人无需登录，可以看到小组名、游戏、日期、玩家昵称、结果、回忆和照片。不会显示账号、记录人或其他组内内容。</p>
      <p v-if="shareActive && !shareURL" class="j-notice">当前已有公开链接。重新生成后，旧链接会立即失效。</p>
      <button class="d-button full" :disabled="shareBusy" @click="generateShare">{{ shareActive ? '重新生成分享链接' : '生成分享链接' }}</button>
      <label v-if="shareURL" class="d-field">分享链接<input :value="shareURL" readonly @focus="($event.target as HTMLInputElement).select()" /></label>
      <button v-if="shareURL" class="d-button secondary full" @click="copyShare">{{ copied ? '已复制' : '复制链接' }}</button>
      <button v-if="shareActive" class="d-text-link j-share-revoke" :disabled="shareBusy" @click="revokeShare">撤销公开链接</button>
      <p v-if="shareError" class="j-error">{{ shareError }}</p>
    </Modal>
    <Modal v-if="modal === 'photo'" title="那天的桌边时光" wide @close="modal = ''"><img class="d-lightbox" :src="photoURL(groupId, fullPhoto)" alt="对局回忆大图" /></Modal>
  </template>
</template>
