...
function save(round: Round, key: string) { return run(async () => { saved.value = await createRound(groupId, round, key) }) }
</script>

<template>
<p v-if="notice" class="j-notice" role="status">{{ notice }}</p><RoundForm v-if="snapshot && user" :snapshot="snapshot" :user="user" :initial-game="typeof route.query.game === 'string' ? route.query.game : undefined" :saving="busy" :error="error" :saved="saved" :add-item="addItem" :upload-photo="upload" @submit="save" @again="saved = null" />
</template>
