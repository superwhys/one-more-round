...
当前已接入真实持久化与业务接口：邮箱验证码和试用邀请、小组创建/加入/切换、成员管理与玩家关联、手动游戏、三种模式记局、草稿、编辑删除、照片授权访问、时间线和筛选统计。正式入口为 `/`；`/design` 保留独立示例产品稿，其数据不进入业务库。
...
`web/src/routers/index.ts` 统一创建路由与处理登录/小组入口；正式业务页和 `/design` 产品稿各自对应独立的 `views/**/*.vue`，不再通过 URL 分支渲染多种页面。新增与编辑对局分别编排提交，共用 `RoundForm`；回顾、桌游详情和玩家详情共用时间线及统计组件。

```text
web/src/
├── api/          # request.ts 统一请求；auth/group/game/round/photo 按业务封装
├── components/   # common、layout、rounds、groups、design 等可复用 UI
├── routers/      # 唯一路由实例、守卫及产品稿路由模块
├── types/        # 服务端 DTO、共享业务类型；无请求和存储副作用
├── utils/        # 日期、结果展示、草稿清理、错误文本
├── views/        # 登录、小组、回顾、桌游、玩家、对局和产品稿独立页面
├── composables/  # 小组上下文、异步请求与分页等共享响应式逻辑
├── stores/       # 会话/当前小组；产品稿示例状态独立存放
├── styles/       # 共用视觉规则与表单样式
...
跨层导入使用 `@/`。表单草稿仍按账号、小组和记录隔离；会话凭据仍由服务端 HttpOnly Cookie 管理。产品稿示例数据位于 `stores/designPreview.ts`，正式业务页面不依赖它。旧的未接入占位稿保留在 `components/preview/`，不注册为业务页面。
