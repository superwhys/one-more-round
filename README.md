...
- **BGG 关联与合并**：手动桌游可关联 BGG 条目；同组已有该 BGG 条目时，由组主确认合并并保留历史对局。
- **小组想玩清单**：成员从本组桌游架标记想玩的游戏，聚会时可从清单直接进入记局。
...
BGG 搜索、导入、手动条目关联与组主确认合并已接入。服务端使用 `app.bgg.token` 调用 XML API，结果缓存一小时；未配置、授权失败或外部不可用时仍可手动添加。BGG 返回的封面地址会在桌游架与详情页加载，缺封面时使用统一占位图。想玩清单和聚会相册也已接入当前源码。

BGG 资料展示使用其[官方 “Powered by BGG” 标识](https://drive.google.com/file/d/1iQTdLUoxa8NM8T1tJEJj1GA6iPzpZ-AR/view)，居中放在页面页脚并链接回 BoardGameGeek；标识作为本地前端资源随二进制嵌入。

本地配置令牌已只读验证 BGG 搜索与详情接口返回成功；新页面的真实 BGG 操作仍需在部署环境验收。**尚未验证：**60 秒录入目标，以及 2—3 个真实小组的两周试用。当前工作区改动尚未部署。
...
原生小程序在 [`mini/`](mini/README.md)，使用同一后端。运行 `make mini-check` 检查并构建，微信登录与已有邮箱绑定的配置和验证说明见小程序 README。
