...
func (r *commentRepository) ListByRound(
	ctx context.Context,
	groupID, roundID string,
	offset, limit int,
) ([]*diary.Comment, int, error) {
	q := queryOf(r.db).RoundComment
	rows, total, err := q.WithContext(ctx).
		Where(q.GroupID.Eq(groupID), q.RoundID.Eq(roundID)).
		Order(q.Created, q.ID).
...
func (r *commentRepository) Get(
	ctx context.Context,
	groupID, roundID, id string,
) (*diary.Comment, error) {
	q := queryOf(r.db).RoundComment
	row, err := q.WithContext(ctx).
		Where(q.GroupID.Eq(groupID), q.RoundID.Eq(roundID), q.ID.Eq(id)).
		Take()
	if err != nil {
...
func (r *commentRepository) Save(ctx context.Context, c *diary.Comment) error {
	return mapErr(
		queryOf(r.db).RoundComment.WithContext(ctx).Create(mapper.CommentDomainToModel(c)),
	)
}
...
func (r *commentRepository) Delete(ctx context.Context, groupID, roundID, id string) error {
	...
	_, err := q.WithContext(ctx).
		Where(q.GroupID.Eq(groupID), q.RoundID.Eq(roundID)).
		Where(q.WithContext(ctx).Where(q.ID.Eq(id)).Or(q.ParentID.Eq(id))).
		Delete()
	return mapErr(err)
}
...
func (r *commentRepository) DeleteByRound(ctx context.Context, groupID, roundID string) error {
	...
	if _, err := queries.CommentIdempotency.WithContext(ctx).
		Where(queries.CommentIdempotency.GroupID.Eq(groupID), queries.CommentIdempotency.RoundID.Eq(roundID)).
		Delete(); err != nil {
		return mapErr(err)
	}
...
func (r *commentIdempotencyRepository) Get(
	ctx context.Context,
	groupID, userID, roundID, key string,
) (string, string, error) {
	q := queryOf(r.db).CommentIdempotency
	row, err := q.WithContext(ctx).
		Where(q.GroupID.Eq(groupID), q.UserID.Eq(userID), q.RoundID.Eq(roundID), q.RequestKey.Eq(key)).
		Take()
	if err != nil {
		return "", "", mapErr(err)
	}
...
func (r *commentIdempotencyRepository) Create(
	ctx context.Context,
	groupID, userID, roundID, key, hash, commentID string,
) error {
	return mapErr(
		queryOf(r.db).CommentIdempotency.WithContext(ctx).Create(&models.CommentIdempotency{
			GroupID:    groupID,
			UserID:     userID,
			RoundID:    roundID,
			RequestKey: key,
