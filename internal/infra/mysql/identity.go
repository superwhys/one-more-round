...
func (r *userRepository) GetBySessionToken(ctx context.Context, hash string) (*identity.User, error) {
	q := queryOf(r.db)
	u, session := q.User, q.Session
	m, err := u.WithContext(ctx).Select(u.ALL).Join(session, session.UserID.EqCol(u.ID)).Where(session.Hash.Eq(hash), session.Expires.Gt(time.Now().UTC())).Take()
	if err != nil {
		return nil, mapErr(err)
	}
	return r.converter.UserModelToDomain(m), nil
...
func (r *verifyCodeRepository) Get(ctx context.Context, email string) (*identity.Challenge, error) {
	...
	if err := c.WithContext(ctx).Clauses(clause.Insert{Modifier: "IGNORE"}).Create(&models.Challenge{Email: email, Expires: epoch, Sent: epoch}); err != nil {
		return nil, mapErr(err)
	}
	m, err := c.WithContext(ctx).Where(c.Email.Eq(email)).Clauses(clause.Locking{Strength: "UPDATE"}).Take()
	if err != nil {
		return nil, mapErr(err)
	}
...
func (r *rateRepository) Hit(ctx context.Context, id string, now time.Time, limit int) error {
	q := queryOf(r.db).Rate
	if err := q.WithContext(ctx).Clauses(clause.Insert{Modifier: "IGNORE"}).Create(&models.Rate{ID: id, Starts: now}); err != nil {
		return mapErr(err)
	}
	...
	_, err = q.WithContext(ctx).Where(q.ID.Eq(id)).UpdateSimple(q.Starts.Value(m.Starts), q.Hits.Value(m.Hits+1))
	return mapErr(err)
}
...
func (r *trialRepository) Consume(ctx context.Context, hash string, now time.Time) error {
	...
	res, err := q.WithContext(ctx).Where(q.Hash.Eq(hash), q.Consumed.Is(false), q.Expires.Gt(now)).UpdateSimple(q.Consumed.Value(true))
	if err != nil {
		return mapErr(err)
	}
