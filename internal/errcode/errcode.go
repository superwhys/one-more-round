...
var (
	...
	ErrWechatUnavailable    = New(CodeUnavailable, 503, "微信登录尚未配置，请联系管理员")
	ErrWechatLogin          = New(CodeBadGateway, 502, "微信登录暂时不可用，请稍后重试")
	ErrWechatCode           = ErrBadRequest.WithMessage("微信登录凭证无效，请重新登录")
	ErrWechatBound          = ErrConflict.WithMessage("该账号已经绑定其他微信，不能重复绑定")
	ErrEmailAccountConflict = ErrConflict.WithMessage("该邮箱已有独立账号，暂不支持合并；请使用原账号登录")
	ErrEmailBound           = ErrConflict.WithMessage("当前账号已经绑定邮箱，暂不支持更换")
	ErrWechatRequired       = ErrForbidden.WithMessage("请先使用微信登录后再绑定邮箱")
